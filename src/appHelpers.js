...
const getHelperFunctions = (user, setUser, setMessage, setToken) => {
  async function logIn({ username, password }) {
    try {
      let loginRes = await JoblyApi.doLogin(username, password);
      if (loginRes) {
        let newUser = await JoblyApi.getUser(username);
        setToken(loginRes);
        setUser(newUser);
        setMessage({
  ...
  async function signUp(signUpData) {
    ...
    try {
      let newUserRes = await JoblyApi.createUser(
        username,
        password,
        firstName,
        lastName,
      ...
      setMessage({
        text: `Welcome to Jobly, ${username}!`,
        variant: "success",
      });
    } catch (e) {
      console.log(e);
  ...
  async function editUser(editUserData) {
    ...
    try {
      let editRes = await JoblyApi.updateUser(
        username,
        firstName,
        lastName,
        email,
